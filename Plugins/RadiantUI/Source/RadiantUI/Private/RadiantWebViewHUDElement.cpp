...
void URadiantWebViewHUDElement::SetSlateVisibility()
{
	...
	{
		SWidget->SetVisibility((InputMode >= ERadiantHUDElementInputMode::MouseOnly) ? EVisibility::Visible : EVisibility::HitTestInvisible);
	}
}

void URadiantWebViewHUDElement::CallJavaScriptFunction(const FString& HookName, UObject* Parameters)
{
	if (!HookName.IsEmpty() && WebView.IsValid())
	{
		if (Parameters)
		{
		...
		{
			FTCHARToUTF8 Convert(*HookName);
			WebView->CallJavaScriptFunction(Convert.Get(), NULL);
		}
	}
}
